...
```flow
 st=>start: Start
 op=>operation: Java后端
 cond=>condition: 是否有文件?
 ishave=>operation: 有文件(.sh/.py/.jar/hdfs/sql/MR/flinkSQL/Spark)
 unhave=>operation: 没有文件
 fileup=>operation: 文件上传
 toshell=>operation: 执行shell
 res=>operation: 任务回调
 e=>end
 
 st->op->cond
 cond(yes)->ishave->toshell
 cond(no)->unhave->fileup->toshell
 toshell->res->e
```
