...
FROM (
    SELECT 
        ...
        cate_id, 
        brand_id, 
        @rank := IF(@prev_dt = DATE(create_time) AND @prev_cate_id = cate_id AND @prev_brand_id = brand_id, @rank + 1, 1) AS rank,
        @prev_dt := DATE(create_time),
        @prev_cate_id := cate_id,
        @prev_brand_id := brand_id
    FROM 
        t_price_data_business_match_result, 
        (SELECT @rank := 0, @prev_dt := NULL, @prev_cate_id := NULL, @prev_brand_id := NULL) AS vars
    WHERE 
        cate_id = 101 and DATE(create_time) = '2024-03-08'
    ORDER BY 
        DATE(create_time), cate_id, brand_id, price DESC
) AS subquery
WHERE rank <= 10
ORDER BY dt, brand_id DESC, rank;
```
因为mysql5的版本不支持row_number() over(partition by … order by …) 的写法，所以在mysql5中，可以用变量保存行号进行类似的操作
> 解释：
> 首先，声明出初始化的几个变量值
>
> ```mysql
> FROM t_price_data_business_match_result, 
>    (SELECT @rank := 0, @prev_dt := NULL, @prev_cate_id := NULL, @prev_brand_id := NULL) AS vars 	
> ```
> 1. **DATE(create_time), cate_id, brand_id, price DESC**： 先按 定好的排序分组规则将数据行排序完毕。
> 2. **@rank := IF(...)**：@rank 会根据前一行的 create_time、cate_id 和 brand_id 判断是否属于同一组，如果是，则排名递增；如果不是，则重置为 1（每组从 1 开始排名）。
> 3. **@prev_dt := DATE(create_time), @prev_cate_id := cate_id, @prev_brand_id := brand_id**：更新 @prev_dt、@prev_cate_id 和 @prev_brand_id 为当前行的值，以便与下一行比较，判断是否在同一分组。
> 4. **WHERE rank <= 10**： 过滤出每组的前 10 名记录。你可以根据实际需要调整这个条件（例如，rank <= 5）。

**由此可以实现类似row_number() over(partition by … order by …) 的开窗的写法**
